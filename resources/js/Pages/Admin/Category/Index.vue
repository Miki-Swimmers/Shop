    ...
    <div>
        ...
        <div>
            <table class="border-collapse table-auto w-full text-sm">
                ...
                <!-- главное: рендерим из categoriesData -->
                <tr v-for="cat in categoriesData" :key="cat.id">
                    <td class="border-b bg-white border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400">{{ cat.id }}</td>
                    <td class="border-b bg-white border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400">
                        <Link :href="route('admin.categories.show', cat.id)">{{ cat.title }}</Link>
                    </td>
                    <td class="border-b bg-white border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400 flex">
                        ...
                        <!-- тут: передаём cat -->
                        <svg @click.prevent="deleteCategory(cat)" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="text-red-600 size-4 cursor-pointer">
